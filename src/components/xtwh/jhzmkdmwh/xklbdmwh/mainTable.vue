      ...
      deleteData(dm){
            ...
            this.axios.get('jwc/DmXklb/delete?xklbdm='+dm)
                .then(function (response) {
                    _this.$message({ message: '成功删除学科类别: '+response.data, type: 'success' });
                    _this.getData();
                })
                .catch(function (error) {
                    console.log(error);
                    _this.$message({ message: '删除学科类别失败: '+error, type: 'error' });
                });
      }, 
      getData(){
        var _this=this;
        _this.loading = true;
        //需要处理异步请求的问题
        this.axios.get('jwc/DmXklb/getAll')
            .then(function (response) {
                //将response获得的数据进行处理
                //将获取到的数据以数组形式传递出去
                var dataList=response.data;
                _this.tableData=dataList;
                _this.$notify({title:"获取学科类别", message:"获取学科类别成功 ("+response.data.length+')', type:"success"});
                _this.loading = false;
            })
            .catch(function (error) {
                console.log(error);
                _this.$notify({title:"获取学科类别", message:"获取学科类别失败: "+error, type:"error"});
                _this.loading = false;
            });
      }
    }
